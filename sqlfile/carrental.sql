...
---
--- Trigger for tblbooking
--Automatically update the Status of a booking when a new record is inserted.
---
DELIMITER $$

CREATE TRIGGER confirm_all_bookings
AFTER INSERT ON tblbooking
FOR EACH ROW
BEGIN
    -- Automatically set Status to '1' for the new booking
    UPDATE tblbooking
    SET Status = 1
    WHERE id = NEW.id;
END $$
...
VALUES ('admin', MD5('admin123')); -- Replace MD5 with a stronger hash like bcrypt in production
...
Here is what makes AHNA self-drive car rental service your first choice for renting self drive cars anywhere in India:
...
---Trigger for tblvehicles
---Track updates to vehicle prices in an audit table.

-- Create an audit table for vehicle price changes
CREATE TABLE VehiclePriceAudit (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
CREATE TRIGGER before_vehicle_price_update
BEFORE UPDATE ON tblvehicles
FOR EACH ROW
BEGIN
    IF OLD.PricePerDay != NEW.PricePerDay THEN
        INSERT INTO VehiclePriceAudit (VehicleId, OldPrice, NewPrice)
        VALUES (OLD.id, OLD.PricePerDay, NEW.PricePerDay);
    END IF;
END $$
...
-----------Procedure to Add a New Vehicle: Insert a new vehicle and automatically log the operation.------------
DELIMITER $$

CREATE PROCEDURE AddNewVehicle (
    IN VehicleTitle VARCHAR(255),
    IN Brand INT,
    IN Overview TEXT,
...
BEGIN
    INSERT INTO tblvehicles (
        ...
        VehicleTitle, Brand, Overview, Price, Fuel, ModelYear, Seats
    );

    -- Log the addition
    INSERT INTO VehiclePriceAudit (VehicleId, OldPrice, NewPrice)
    VALUES (LAST_INSERT_ID(), NULL, Price);
END $$

DELIMITER ;
---------- Procedure to Fetch All Bookings for a User: Retrieve booking details for a given user email.----------
...
BEGIN
    SELECT tblbooking.id, tblvehicles.VehiclesTitle, tblbooking.FromDate, tblbooking.ToDate, tblbooking.Status
    FROM tblbooking
